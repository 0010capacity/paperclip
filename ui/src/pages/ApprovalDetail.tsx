...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approval_detail.approval") },
    ]);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approval_detail.approval_confirmed")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approval_detail.approval_confirmed_desc")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("approval_detail.decision_note")}: {approval.decisionNote}</p>
          )}
        </div>
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("approval_detail.linked_issues_desc")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approval_detail.delete_agent_confirm"))) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approval_detail.comments", { count: comments?.length ?? 0 })}</h3>
        ...
        <Textarea
          ...
          placeholder={t("approval_detail.add_comment_placeholder")}
          rows={3}
        />
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("approval_detail.posting") : t("approval_detail.post_comment")}
          </Button>
        </div>
      </div>
    </div>

...
export function InviteLandingPage() {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("invite.not_available")}</h1>
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("invite.bootstrap_complete_desc")}
          </p>
  ...
  if (result?.kind === "join") {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("invite.join_request_submitted")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("invite.join_request_pending")}
          </p>
          ...
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("invite.one_time_claim")}</p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? t("invite.bootstrap_title") : t("invite.join_title")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">{t("invite.invite_expires")} {dateTime(invite.expiresAt)}.</p>
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {adapterLabels[type]}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("invite.coming_soon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            ...
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("invite.sign_in_create")}</Link>
              </Button>
            </div>
          </div>
        )}
        ...
        >
          {acceptMutation.isPending
            ? t("invite.submitting")
            : invite.inviteType === "bootstrap_ceo"
              ? t("invite.accept_bootstrap")
              : t("invite.submit_join")}
        </Button>
      </div>
    </div>
  );

    ...
    >
      ...
      >
        ...
        <div className="px-4 pb-3 space-y-3 border-t border-border">
          <div className="pt-3">
            <p className="text-sm font-medium">{t("project.workspace_title")}</p>
            <p className="text-xs text-muted-foreground">{t("project.workspace_hint")}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                {t("project.local_folder")}
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t("project.local_folder_hint")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("project.github_repo_hint")}</p>
            </button>
            <button
              type="button"
              className={cn(
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("project.both_hint")}</p>
            </button>
          </div>

          {(workspaceSetup === "local" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("project.local_folder_label")}</label>
              <div className="flex items-center gap-2">
                <input
                  className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                  value={workspaceLocalPath}
                  onChange={(e) => setWorkspaceLocalPath(e.target.value)}
                  placeholder={t("project.local_folder_placeholder")}
                />
                <ChoosePathButton />
              </div>
            </div>
          ...
          {(workspaceSetup === "repo" || workspaceSetup === "both") && (
            <div className="rounded-md border border-border p-2">
              <label className="mb-1 block text-xs text-muted-foreground">{t("project.github_url_label")}</label>
              <input
                className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs outline-none"
                value={workspaceRepoUrl}
                onChange={(e) => setWorkspaceRepoUrl(e.target.value)}
                placeholder={t("project.github_url_placeholder")}
              />
            </div>
          )}
          {workspaceError && (
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("project.add_goal") : t("project.goal")}
              </button>
            </PopoverTrigger>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("project.target_date")}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("project.failed_create")}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("project.creating") : t("project.create_project")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
